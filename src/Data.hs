...
update_data update seq_id (Engine widget window window_map request key main_id start_id count_id time)=let (combined_id,single_id)=get_widget_id_widget seq_id start_id widget in Engine (error_update_update "set_data: error 1" "set_data: error 2" combined_id single_id (update_data_a update) widget) window window_map request key main_id start_id count_id time
