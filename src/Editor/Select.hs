...
min_select_all seq_seq_char typesetting block_number max_row (Cursor_single {})=let number_block_start=start_select seq_seq_char typesetting block_number in let (number_block_end,number_char_end)=end_select seq_seq_char typesetting block_number in if max_row==0&&number_char_end==0 then Nothing else Just (Cursor_double True 0 number_block_start 0 number_block_start max_row number_block_end number_char_end number_block_end)
...
max_select_all seq_seq_char typesetting block_number max_row (Cursor_single {})=let number_block_start=start_select seq_seq_char typesetting block_number in let (number_block_end,number_char_end)=end_select seq_seq_char typesetting block_number in if max_row==0&&number_char_end==0 then Nothing else Just (Cursor_double False 0 number_block_start 0 number_block_start max_row number_block_end number_char_end number_block_end)
...
min_select_row_all seq_seq_char typesetting block_number (Cursor_single cursor_row _ _ _)=let (number_block_start,number_block_end,number_char_end)=select_row seq_seq_char typesetting block_number cursor_row in if number_char_end==0 then Just (cursor_row,Nothing) else Just (cursor_row,Just (Cursor_double True cursor_row number_block_start 0 number_block_start cursor_row number_block_end number_char_end number_block_end))
...
max_select_row_all seq_seq_char typesetting block_number (Cursor_single cursor_row _ _ _)=let (number_block_start,number_block_end,number_char_end)=select_row seq_seq_char typesetting block_number cursor_row in if number_char_end==0 then Just (cursor_row,Nothing) else Just (cursor_row,Just (Cursor_double False cursor_row number_block_start 0 number_block_start cursor_row number_block_end number_char_end number_block_end))
...
select_left seq_seq_char typesetting block_number (Cursor_single cursor_row cursor_block cursor_char cursor_click)
    |cursor_row==0=if cursor_char==0 then Just (0,Nothing) else let new_cursor_char=cursor_char-1 in let new_cursor_block=left_select seq_seq_char 0 cursor_block new_cursor_char in Just (0,Just (Cursor_double True 0 new_cursor_block new_cursor_char new_cursor_block 0 cursor_block cursor_char cursor_click))
    |otherwise=if cursor_char==0 then let new_cursor_row=cursor_row-1 in let (new_cursor_block,new_cursor_char)=left_select_start seq_seq_char typesetting block_number new_cursor_row in Just (new_cursor_row,Just (Cursor_double True new_cursor_row new_cursor_block new_cursor_char new_cursor_block cursor_row cursor_block 0 cursor_click)) else let new_cursor_char=cursor_char-1 in let new_cursor_block=left_select seq_seq_char cursor_row cursor_block new_cursor_char in Just (cursor_row,Just (Cursor_double True cursor_row new_cursor_block new_cursor_char new_cursor_block cursor_row cursor_block cursor_char cursor_click))
...
select_right seq_seq_char typesetting block_number max_row (Cursor_double cursor_where cursor_row_start cursor_block_start cursor_char_start cursor_click_start cursor_row_end cursor_block_end cursor_char_end cursor_click_end)
    |cursor_where=if cursor_row_start==max_row
        then case DS.lookup max_row seq_seq_char of
            ...
            Just (seq_char,_,_,_)->let new_cursor_char=cursor_char_start+1 in if new_cursor_char==cursor_char_end then Just (max_row,Just (Cursor_single max_row cursor_block_end cursor_char_end cursor_click_end)) else let new_cursor_block=right_select seq_char cursor_block_start cursor_char_start in Just (max_row,Just (Cursor_double True max_row new_cursor_block new_cursor_char new_cursor_block max_row cursor_block_end cursor_char_end cursor_click_end))
        else case DS.lookup cursor_row_start seq_seq_char of
            Nothing->error "select_right: error 4"
            Just (seq_char,_,char_number,_)->if cursor_char_start==char_number then let new_cursor_row=cursor_row_start+1 in if new_cursor_row==cursor_row_end&&cursor_char_end==0 then Just (cursor_row_end,Just (Cursor_single cursor_row_end cursor_block_end 0 cursor_click_end)) else let new_cursor_block=right_select_end seq_seq_char typesetting block_number new_cursor_row in Just (new_cursor_row,Just (Cursor_double True new_cursor_row new_cursor_block 0 new_cursor_block cursor_row_end cursor_block_end cursor_char_end cursor_click_end)) else let new_cursor_char=cursor_char_start+1 in if cursor_row_end==cursor_row_start&&new_cursor_char==cursor_char_end then Just (cursor_row_end,Just (Cursor_single cursor_row_end cursor_block_end cursor_char_end cursor_click_end)) else let new_cursor_block=right_select seq_char cursor_block_start cursor_char_start in Just (cursor_row_start,Just (Cursor_double True cursor_row_start new_cursor_block new_cursor_char new_cursor_block cursor_row_end cursor_block_end cursor_char_end cursor_click_end))
...
click_select seq_seq_char typesetting block_number cursor_row cursor_click=case DS.lookup cursor_row seq_seq_char of
    Nothing->error "click_select: error 1"
    Just (seq_char,number,_,_)->from_cursor_click seq_char cursor_click (typesetting_left typesetting number block_number) 0

up_select_start::DS.Seq (DS.Seq (Char,Int,FCT.CInt),Int,Int,Bool)->Typesetting->Int->Int->Int
up_select_start seq_seq_char typesetting block_number cursor_row=case DS.lookup cursor_row seq_seq_char of
    Nothing->error "up_select_start: error 1"
    Just (_,number,_,_)->typesetting_left typesetting number block_number
...
select_up seq_seq_char typesetting block_number (Cursor_single cursor_row cursor_block cursor_char cursor_click)
    |cursor_row==0=if cursor_char==0 then Just (0,Nothing) else let new_cursor_block=up_select_start seq_seq_char typesetting block_number 0 in Just (0,Just (Cursor_double True 0 new_cursor_block 0 new_cursor_block 0 cursor_block cursor_char cursor_click))
    |otherwise=let new_cursor_row=cursor_row-1 in let (new_cursor_block,new_cursor_char)=click_select seq_seq_char typesetting block_number new_cursor_row cursor_click in Just (new_cursor_row,Just (Cursor_double True new_cursor_row new_cursor_block new_cursor_char cursor_click cursor_row cursor_block cursor_char cursor_click))
select_up seq_seq_char typesetting block_number (Cursor_double cursor_where cursor_row_start cursor_block_start cursor_char_start cursor_click_start cursor_row_end cursor_block_end cursor_char_end cursor_click_end)
    |cursor_where=if cursor_row_start==0 then if cursor_char_start==0 then Just (0,Nothing) else let new_cursor_block=up_select_start seq_seq_char typesetting block_number 0 in Just (0,Just (Cursor_double True 0 new_cursor_block 0 new_cursor_block cursor_row_end cursor_block_end cursor_char_end cursor_click_end)) else let new_cursor_row=cursor_row_start-1 in let (new_cursor_block,new_cursor_char)=click_select seq_seq_char typesetting block_number new_cursor_row cursor_click_start in Just (new_cursor_row,Just (Cursor_double True new_cursor_row new_cursor_block new_cursor_char cursor_click_start cursor_row_end cursor_block_end cursor_char_end cursor_click_end))
    |otherwise=if cursor_row_end==0 then if cursor_char_start==0 then Just (0,Just (Cursor_single 0 cursor_block_start 0 cursor_click_start)) else let new_cursor_block=up_select_start seq_seq_char typesetting block_number 0 in Just (0,Just (Cursor_double True 0 new_cursor_block 0 cursor_click_end 0 cursor_block_start cursor_char_start cursor_click_start)) else let new_cursor_row=cursor_row_end-1 in let (new_cursor_block,new_cursor_char)=click_select seq_seq_char typesetting block_number new_cursor_row cursor_click_end in if new_cursor_row==cursor_row_start&&new_cursor_char==cursor_char_start then Just (cursor_row_start,Just (Cursor_single cursor_row_start cursor_block_start cursor_char_start cursor_click_start)) else if (new_cursor_row,new_cursor_char)<(cursor_row_start,cursor_char_start) then Just (new_cursor_row,Just (Cursor_double True new_cursor_row new_cursor_block new_cursor_char cursor_click_end cursor_row_start cursor_block_start cursor_char_start cursor_click_start)) else Just (new_cursor_row,Just (Cursor_double False cursor_row_start cursor_block_start cursor_char_start cursor_click_start new_cursor_row new_cursor_block new_cursor_char cursor_click_end))
...
select_down seq_seq_char typesetting block_number max_row (Cursor_single cursor_row cursor_block cursor_char cursor_click)
    |cursor_row==max_row=case DS.lookup max_row seq_seq_char of
        Nothing->error "select_down: error 1"
        Just (_,number,char_number,_)->if cursor_char==char_number then Just (max_row,Nothing) else let new_cursor_block=typesetting_right typesetting number block_number in Just (max_row,Just (Cursor_double False max_row cursor_block cursor_char cursor_click max_row new_cursor_block char_number new_cursor_block))
    |otherwise=let new_cursor_row=cursor_row+1 in let (new_cursor_block,new_cursor_char)=click_select seq_seq_char typesetting block_number new_cursor_row cursor_click in Just (new_cursor_row,Just (Cursor_double False cursor_row cursor_block cursor_char cursor_click new_cursor_row new_cursor_block new_cursor_char cursor_click))
select_down seq_seq_char typesetting block_number max_row (Cursor_double cursor_where cursor_row_start cursor_block_start cursor_char_start cursor_click_start cursor_row_end cursor_block_end cursor_char_end cursor_click_end)
    |cursor_where=if cursor_row_start==max_row
        then case DS.lookup max_row seq_seq_char of
            Nothing->error "select_down: error 2"
            Just (_,number,char_number,_)->if cursor_char_end==char_number then Just (max_row,Just (Cursor_single max_row cursor_block_end char_number cursor_click_end)) else let new_cursor_block=typesetting_right typesetting number block_number in Just (max_row,Just (Cursor_double False max_row cursor_block_end cursor_char_end cursor_click_end max_row new_cursor_block char_number new_cursor_block))
        else let new_cursor_row=cursor_row_start+1 in let (new_cursor_block,new_cursor_char)=click_select seq_seq_char typesetting block_number new_cursor_row cursor_click_start in if new_cursor_row==cursor_row_end&&new_cursor_char==cursor_char_end then Just (cursor_row_end,Just (Cursor_single cursor_row_end cursor_block_end cursor_char_end cursor_click_end)) else if (cursor_row_end,cursor_char_end)<(new_cursor_row,new_cursor_char) then Just (new_cursor_row,Just (Cursor_double False cursor_row_end cursor_block_end cursor_char_end cursor_click_end new_cursor_row new_cursor_block new_cursor_char cursor_click_start)) else Just (new_cursor_row,Just (Cursor_double True new_cursor_row new_cursor_block new_cursor_char cursor_click_start cursor_row_end cursor_block_end cursor_char_end cursor_click_end))
    |otherwise=if cursor_row_end==max_row
        then case DS.lookup max_row seq_seq_char of
