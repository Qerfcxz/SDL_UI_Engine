...
create_text_trigger_a seq_id start_id event widget=let (combined_id,single_id)=get_widget_id_widget seq_id start_id widget in let intmap_combined_widget=error_lookup "create_text_trigger_a: error 1" combined_id widget in case error_lookup "create_text_trigger_a: error 2" single_id intmap_combined_widget of
    combined_widget->case create_text_trigger_b event combined_widget of
        Nothing->widget
        Just new_combined_widget->DIS.insert combined_id (DIS.insert single_id new_combined_widget intmap_combined_widget) widget

create_text_trigger_b::GS.HasCallStack=>Event->Combined_widget a->Maybe (Combined_widget a)
create_text_trigger_b event (Leaf_widget next_id (Text window_id row max_row render select find design_delta_height design_left design_right design_up design_down delta_height left right up down seq_paragraph seq_row text_binding))=case text_binding of
    Empty_text_binding->Nothing
    Text_binding wheel up_press down_press min_press max_press down_click->if select
        then case event of
            ...
            _->Nothing
create_text_trigger_b _ _=error "create_text_trigger_b: error 1"