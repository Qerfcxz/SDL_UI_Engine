...
create_text_trigger::Bool->DSet.Set (Press,DSet.Set Key)->DSet.Set (Press,DSet.Set Key)->DSet.Set (Press,DSet.Set Key)->DSet.Set (Press,DSet.Set Key)->DSet.Set (Click,Mouse)->(Event->Engine a->Id)->DSeq.Seq Int->DSeq.Seq (DSeq.Seq Int)->Engine a->IO (Engine a)
create_text_trigger wheel up_press down_press min_press max_press down_click next_id seq_id seq_seq_id=create_widget seq_id (Leaf_widget_request next_id (Trigger_request (\event (Engine widget window window_map request key count_id start_id main_id)->let new_widget=DF.foldl' (\this_widget this_seq_id->create_text_trigger_a wheel up_press down_press min_press max_press down_click this_seq_id start_id event this_widget) widget seq_seq_id in Engine new_widget window window_map request key count_id start_id main_id)))
