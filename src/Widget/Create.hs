...
create_single_widget start_id window (Text_request window_id find delta_height left right up down seq_paragraph) widget=case error_lookup "create_single_widget: error 5" window_id window of
    (Window _ _ renderer _ _ x y design_size size)->let new_delta_height=div (delta_height*size) design_size in do
        ...
        let new_up=y+div (up*size) design_size in let new_down=y+div (down*size) design_size in let max_row=find_max seq_row new_up new_down in return (Text window_id 0 max_row False False find delta_height left right up down new_delta_height (x+div (left*size) design_size) (x+div (right*size) design_size) new_up new_down seq_paragraph seq_row)
create_single_widget start_id window (Editor_request window_id block_number font_size path find typesetting text_red text_green text_blue text_alpha cursor_red cursor_green cursor_blue cursor_alpha select_red select_green select_blue select_alpha block_width height delta_height x y extra_width extra_height ime_left ime_right ime_up ime_down seq_seq_char) widget=let (window_x,window_y,design_size,size)=get_adaptive_window window_id window in let new_block_width=div (block_width*size) design_size in let (_,_,_,new_font_size,_,_,_,_,_,_,_,font_height,intmap_texture)=find_block_font find widget block_width new_block_width design_size size path start_id font_size in FMA.alloca $ \text_color->do
