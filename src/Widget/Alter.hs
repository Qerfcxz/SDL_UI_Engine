...
alter_widget_b _ _ _ (Bool_data_request bool) this_widget=case this_widget of
    Bool_data {}->return (Bool_data bool)
    _->error "alter_widget_b: error 2"
alter_widget_b _ _ _ (Int_data_request int) this_widget=case this_widget of
    Int_data {}->return (Int_data int)
    _->error "alter_widget_b: error 3"
alter_widget_b _ _ _ (Char_data_request char) this_widget=case this_widget of
...
alter_widget_b _ window _ (Rectangle_request window_id red green blue alpha left right up down) this_widget=case this_widget of
    Rectangle {}->case error_lookup "alter_widget_b: error 12" window_id window of
...
alter_widget_b start_id window widget (Text_request window_id find delta_height left right up down seq_paragraph) this_widget=case this_widget of
    Text _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ seq_row->case error_lookup "alter_widget_b: error 18" window_id window of
        (Window _ _ renderer _ _ x y design_size size)->do
            ...
            let new_up=y+div (up*size) design_size in let new_down=y+div (down*size) design_size in let max_row=find_max new_seq_row new_up new_down in return (Text window_id 0 max_row False False find delta_height left right up down new_delta_height (x+div (left*size) design_size) (x+div (right*size) design_size) new_up new_down seq_paragraph new_seq_row)
    _->error "alter_widget_b: error 19"
alter_widget_b start_id window widget (Editor_request window_id block_number font_size path find typesetting text_red text_green text_blue text_alpha cursor_red cursor_green cursor_blue cursor_alpha select_red select_green select_blue select_alpha block_width height delta_height x y extra_width extra_height ime_left ime_right ime_up ime_down seq_seq_char) this_widget=case this_widget of
    Editor {}->let (window_x,window_y,design_size,size)=get_adaptive_window window_id window in let new_block_width=div (block_width*size) design_size in let (_,_,_,new_font_size,_,_,_,_,_,_,_,font_height,intmap_texture)=find_block_font find widget block_width new_block_width design_size size path start_id font_size in FMA.alloca $ \text_color->do
        ...
        return (Editor window_id block_number (fromIntegral (div (div ((height+delta_height)*size) design_size) (font_height+new_delta_height))) 0 font_size new_font_size False path find typesetting text_red text_green text_blue text_alpha cursor_red cursor_green cursor_blue cursor_alpha select_red select_green select_blue select_alpha height block_width delta_height x y extra_width extra_height ime_left ime_right ime_up ime_down font_height new_block_width new_delta_height (window_x+div (x*size) design_size-div (fromIntegral block_number*new_block_width) 2) (window_y+div (y*size) design_size-half_height) (window_x+div ((x-extra_width)*size) design_size-half_width) (window_x+div ((x+extra_width)*size) design_size+half_width) (window_y+div ((y-extra_height)*size) design_size-half_height) (window_y+div ((y+extra_height)*size) design_size+half_height) (window_x+div ((x+ime_left)*size) design_size) (window_x+div ((x+ime_right)*size) design_size) (window_y+div ((y+ime_up)*size) design_size) (window_y+div ((y+ime_down)*size) design_size) Cursor_none (from_seq_seq_char intmap_texture block_number new_block_width seq_seq_char DS.empty))
