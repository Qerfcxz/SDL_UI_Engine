...
direct_alter_widget::Data a=>Int->Int->Combined_widget_request a->Engine a->IO (Engine a)
direct_alter_widget combined_id single_id combined_widget_request (Engine widget window window_map request key main_id start_id count_id time)=do
    new_widget<-error_update_update_io "direct_alter_widget: error 1" "direct_alter_widget: error 2" combined_id single_id (alter_widget_a start_id window widget combined_widget_request) widget
...
alter_widget seq_id combined_widget_request (Engine widget window window_map request key main_id start_id count_id time)=let (combined_id,single_id)=get_widget_id_widget seq_id start_id widget in do
...
alter_single_widget _ _ _ (List_char_data_request list_char) this_widget=case this_widget of
...
alter_single_widget _ _ _ (Collector_request request) this_widget=case this_widget of
    Collector {}->return (Collector request)
    _->error "alter_single_widget: error 9"
...
alter_single_widget _ window _ (Picture_request window_id path render_flip angle x y width_multiply width_divide height_multiply height_divide) this_widget=case this_widget of
    Picture _ texture _ _ _ _ _ _ _ _ _ _ _ _ _ _->case DIS.lookup window_id window of
        Nothing->error "alter_single_widget: error 14"
        Just (Window _ _ renderer _ _ window_x window_y design_size size)->do
            ...
            CM.when (surface==FP.nullPtr) $ error "alter_single_widget: error 15"
            ...
            CM.when (new_texture==FP.nullPtr) $ error "alter_single_widget: error 16"
...
alter_single_widget start_id window widget (Text_request window_id row find delta_height left right up down seq_paragraph) this_widget=case this_widget of
    Text _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ seq_row->case DIS.lookup window_id window of
        Nothing->error "alter_single_widget: error 18"
    ...
    _->error "alter_single_widget: error 19"
alter_single_widget start_id window widget (Editor_request window_id block_number font_size path find typesetting text_red text_green text_blue text_alpha cursor_red cursor_green cursor_blue cursor_alpha select_red select_green select_blue select_alpha block_width height delta_height x y extra_width extra_height seq_text) this_widget=case this_widget of
    ...
    _->error "alter_single_widget: error 20"