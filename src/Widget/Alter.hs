...
alter_widget_b _ window _ (Geometry_request window_id red green blue alpha geometry_request) this_widget=case this_widget of
    Geometry {}->case error_lookup "alter_widget_b: error 14" window_id window of
        (Window _ _ _ _ _ window_x window_y design_size size)->case geometry_request of
            ...
            Rounded_rectangle_request kind left right up down radius->return (Geometry window_id red green blue alpha (Rounded_rectangle kind left right up down radius (fromIntegral (window_x+div (left*size) design_size)) (fromIntegral (window_x+div (right*size) design_size-1)) (fromIntegral (window_y+div (up*size) design_size)) (fromIntegral (window_y+div (down*size) design_size-1)) (fromIntegral (div (radius*size) design_size))))
            Ellipse_request kind left right up down->return (Geometry window_id red green blue alpha (Ellipse kind left right up down (fromIntegral (window_x+div ((left+right)*size) (2*design_size))) (fromIntegral (window_y+div ((up+down)*size) (2*design_size))) (fromIntegral (div ((right-left)*size) (2*design_size))) (fromIntegral (div ((down-up)*size) (2*design_size)))))
            Polygon_request kind seq_point->return (Geometry window_id red green blue alpha (Polygon kind seq_point (fmap (\(x,y)->(fromIntegral (window_x+div (x*size) design_size),fromIntegral (window_y+div (y*size) design_size))) seq_point) (fromIntegral (DS.length seq_point))))
