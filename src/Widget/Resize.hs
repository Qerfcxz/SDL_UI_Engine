...
update_widget_a seq_id start_id window widget=let (combined_id,single_id)=get_widget_id_widget seq_id start_id widget in case error_lookup_lookup "update_widget_a: error 1" "update_widget_a: error 2" combined_id single_id widget of
    Leaf_widget next_id (Canvas window_id allow intmap_texture)->if allow then return widget else do
        DF.mapM_ (\(_,_,texture)->SRV.destroyTexture texture) intmap_texture
        return (error_replace_replace "update_widget_a: error 3" "update_widget_a: error 4" combined_id single_id (Leaf_widget next_id (Canvas window_id allow DIS.empty)) widget)
    Leaf_widget next_id (Rectangle window_id red green blue alpha left right up down _ _ _ _)->let (x,y,design_size,size)=get_adaptive_window window_id window in return (error_replace_replace "update_widget_a: error 5" "update_widget_a: error 6" combined_id single_id (Leaf_widget next_id (Rectangle window_id red green blue alpha left right up down (x+div (left*size) design_size) (y+div (up*size) design_size) (div ((right-left)*size) design_size) (div ((down-up)*size) design_size))) widget)
    Leaf_widget next_id (Picture window_id texture render_flip angle x y width_multiply width_divide height_multiply height_divide width height _ _ _ _)->let (window_x,window_y,design_size,size)=get_adaptive_window window_id window in let new_width=div (width*width_multiply) width_divide in let new_height=div (height*height_multiply) height_divide in return (error_replace_replace "update_widget_a: error 7" "update_widget_a: error 8" combined_id single_id (Leaf_widget next_id (Picture window_id texture render_flip angle x y width_multiply width_divide height_multiply height_divide width height (window_x+div ((x-div new_width 2)*size) design_size) (window_y+div ((y-div new_height 2)*size) design_size) (div (new_width*size) design_size) (div (new_height*size) design_size))) widget)
    Leaf_widget next_id (Animation window_id count index seq_frame render_flip angle x y width_multiply width_divide height_multiply height_divide)->let (window_x,window_y,design_size,size)=get_adaptive_window window_id window in return (error_replace_replace "update_widget_a: error 9" "update_widget_a: error 10" combined_id single_id (Leaf_widget next_id (Animation window_id count index (fmap (\(Frame texture original_width original_height _ _ _ _)->let new_width=div (original_width*width_multiply) width_divide in let new_height=div (original_height*height_multiply) height_divide in Frame texture original_width original_height (window_x+div ((x-div new_width 2)*size) design_size) (window_y+div ((y-div new_height 2)*size) design_size) (div (new_width*size) design_size) (div (new_height*size) design_size)) seq_frame) render_flip angle x y width_multiply width_divide height_multiply height_divide)) widget)
