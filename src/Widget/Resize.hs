...
update_widget_a seq_id start_id window widget=let (combined_id,single_id)=get_widget_id_widget seq_id start_id widget in case error_lookup_lookup "update_widget_a: error 1" "update_widget_a: error 2" combined_id single_id widget of
    Leaf_widget next_id (Geometry window_id red green blue alpha geometry)->let (window_x,window_y,design_size,size)=get_adaptive_window window_id window in case geometry of
        ...
        Ellipse x y radius_x radius_y _ _ _ _->return (error_replace_replace "update_widget_a: error 5" "update_widget_a: error 6" combined_id single_id (Leaf_widget next_id (Geometry window_id red green blue alpha (Ellipse x y radius_x radius_y (window_x+div (x*size) design_size) (window_y+div (y*size) design_size) (div (radius_x*size) design_size) (div (radius_y*size) design_size)))) widget)
    ...
    Leaf_widget next_id (Editor window_id block_number _ row font_size _ _ path find typesetting text_red text_green text_blue text_alpha cursor_red cursor_green cursor_blue cursor_alpha select_red select_green select_blue select_alpha height block_width delta_height x y extra_width extra_height ime_left ime_right ime_up ime_down _ _ _ _ _ _ _ _ _ _ _ _ _ _ seq_seq_char editor_binding)->let (renderer,window_x,window_y,design_size,size)=get_renderer_with_adaptive_window window_id window in let new_block_width=div (block_width*size) design_size in let (font_next_id,font_combined_id,font_single_id,new_font_size,font_window_id,font_red,font_green,font_blue,font_alpha,font,this_font,_,intmap_texture)=find_block_font find widget block_width new_block_width design_size size path start_id font_size in FMA.alloca $ \text_color->do
        ...
        case seq_seq_char of
            ...
            ((seq_char,_,_,end) DS.:<| other_seq_seq_char)->do
                ...
                let new_delta_height=div (delta_height*size) design_size
                let new_row_number=fromIntegral (div (div ((height+delta_height)*size) design_size) (new_font_height+new_delta_height))
