...
update_widget_a seq_id start_id window widget=let (combined_id,single_id)=get_widget_id_widget seq_id start_id widget in case error_lookup_lookup "update_widget_a: error 1" "update_widget_a: error 2" combined_id single_id widget of
    Leaf_widget next_id (Geometry window_id red green blue alpha geometry)->let (window_x,window_y,design_size,size)=get_adaptive_window window_id window in case geometry of
        Rectangle left right up down _ _ _ _->return (error_replace_replace "update_widget_a: error 3" "update_widget_a: error 4" combined_id single_id (Leaf_widget next_id (Geometry window_id red green blue alpha (Rectangle left right up down (window_x+div (left*size) design_size) (window_y+div (up*size) design_size) (div ((right-left)*size) design_size) (div ((down-up)*size) design_size)))) widget)
        Ellipse left right up down _ _ _ _->return (error_replace_replace "update_widget_a: error 5" "update_widget_a: error 6" combined_id single_id (Leaf_widget next_id (Geometry window_id red green blue alpha (Ellipse left right up down (window_x+div ((left+right)*size) (2*design_size)) (window_y+div ((up+down)*size) (2*design_size)) (div ((right-left)*size) (2*design_size)) (div ((down-up)*size) (2*design_size))))) widget)
