...
create_window_trigger::(DS.Seq Int->Engine a->Event->Id)->DS.Seq Int->DS.Seq Int->Engine a->IO (Engine a)
...
create_widget_trigger::(DS.Seq Int->Engine a->Event->Id)->DS.Seq Int->DIS.IntMap (DS.Seq (DS.Seq Int))->Engine a->IO (Engine a)
...
update_widget_a::GS.HasCallStack=>DS.Seq Int->Int->DIS.IntMap Window->DIS.IntMap (DIS.IntMap (Combined_widget a))->IO (DIS.IntMap (DIS.IntMap (Combined_widget a)))
update_widget_a seq_id start_id window widget=let (combined_id,single_id)=get_widget_id_widget seq_id start_id widget in case error_lookup_lookup "update_widget_a: error 1" "update_widget_a: error 2" combined_id single_id widget of
    Leaf_widget next_id (Geometry window_id red green blue alpha geometry)->let (window_x,window_y,design_size,size)=get_adaptive_window window_id window in case geometry of
        Rectangle kind left right up down _ _ _ _->return (error_replace_replace "update_widget_a: error 3" "update_widget_a: error 4" combined_id single_id (Leaf_widget next_id (Geometry window_id red green blue alpha (Rectangle kind left right up down (window_x+div (left*size) design_size) (window_y+div (up*size) design_size) (div ((right-left)*size) design_size) (div ((down-up)*size) design_size)))) widget)
        Rounded_rectangle kind left right up down radius _ _ _ _ _->return (error_replace_replace "update_widget_a: error 5" "update_widget_a: error 6" combined_id single_id (Leaf_widget next_id (Geometry window_id red green blue alpha (Rounded_rectangle kind left right up down radius (fromIntegral (window_x+div (left*size) design_size)) (fromIntegral (window_x+div (right*size) design_size-1)) (fromIntegral (window_y+div (up*size) design_size)) (fromIntegral (window_y+div (down*size) design_size-1)) (fromIntegral (div (radius*size) design_size))))) widget)
        Ellipse kind left right up down _ _ _ _->return (error_replace_replace "update_widget_a: error 7" "update_widget_a: error 8" combined_id single_id (Leaf_widget next_id (Geometry window_id red green blue alpha (Ellipse kind left right up down (fromIntegral (window_x+div ((left+right)*size) (2*design_size))) (fromIntegral (window_y+div ((up+down)*size) (2*design_size))) (fromIntegral (div ((right-left)*size) (2*design_size))) (fromIntegral (div ((down-up)*size) (2*design_size)))))) widget)
        Polygon kind seq_point _ number->return (error_replace_replace "update_widget_a: error 9" "update_widget_a: error 10" combined_id single_id (Leaf_widget next_id (Geometry window_id red green blue alpha (Polygon kind seq_point (fmap (\(x,y)->(fromIntegral (window_x+div (x*size) design_size),fromIntegral (window_y+div (y*size) design_size))) seq_point) number))) widget)
    Leaf_widget next_id (Picture window_id texture (Similarity render_flip angle x y width_multiply width_divide height_multiply height_divide) width height _ _ _ _)->let (window_x,window_y,design_size,size)=get_adaptive_window window_id window in let new_width=div (width*width_multiply) width_divide in let new_height=div (height*height_multiply) height_divide in return (error_replace_replace "update_widget_a: error 11" "update_widget_a: error 12" combined_id single_id (Leaf_widget next_id (Picture window_id texture (Similarity render_flip angle x y width_multiply width_divide height_multiply height_divide) width height (window_x+div ((x-div new_width 2)*size) design_size) (window_y+div ((y-div new_height 2)*size) design_size) (div (new_width*size) design_size) (div (new_height*size) design_size))) widget)
    Leaf_widget next_id (Animation window_id count index seq_frame (Similarity render_flip angle x y width_multiply width_divide height_multiply height_divide))->let (window_x,window_y,design_size,size)=get_adaptive_window window_id window in return (error_replace_replace "update_widget_a: error 13" "update_widget_a: error 14" combined_id single_id (Leaf_widget next_id (Animation window_id count index (fmap (\(Frame texture original_width original_height _ _ _ _)->let new_width=div (original_width*width_multiply) width_divide in let new_height=div (original_height*height_multiply) height_divide in Frame texture original_width original_height (window_x+div ((x-div new_width 2)*size) design_size) (window_y+div ((y-div new_height 2)*size) design_size) (div (new_width*size) design_size) (div (new_height*size) design_size)) seq_frame) (Similarity render_flip angle x y width_multiply width_divide height_multiply height_divide))) widget)
    Leaf_widget next_id (Text window_id row _ _ select find delta_height left right up down _ _ _ _ _ seq_paragraph seq_row text_binding)->do
        DF.mapM_ clean_row seq_row
        let (renderer,x,y,design_size,size)=get_renderer_with_adaptive_window window_id window
    ...
    Leaf_widget next_id (Editor window_id block_number _ row font_size _ _ path find typesetting text_red text_green text_blue text_alpha cursor_red cursor_green cursor_blue cursor_alpha select_red select_green select_blue select_alpha height block_width delta_height x y extra_width extra_height ime_left ime_right ime_up ime_down _ _ _ _ _ _ _ _ _ _ _ _ _ _ seq_seq_char editor_binding)->let (renderer,window_x,window_y,design_size,size)=get_renderer_with_adaptive_window window_id window in let new_block_width=div (block_width*size) design_size in let (font_next_id,font_combined_id,font_single_id,new_font_size,font_window_id,font_red,font_green,font_blue,font_alpha,font,this_font,_,intmap_texture)=find_block_font find widget block_width new_block_width design_size size path start_id font_size in FMA.alloca $ \text_color->do
        ...
        case seq_seq_char of
            ...
            ((seq_char,_,_,end) DS.:<| other_seq_seq_char)->do
                ...
                let new_delta_height=div (delta_height*size) design_size
                let new_row_number=fromIntegral (div ((height+delta_height)*size) ((new_font_height+new_delta_height)*design_size))
                let half_width=div (fromIntegral block_number*new_block_width) 2
                let half_height=div (height*size) (2*design_size)
                return (error_replace_replace "update_widget_a: error 18" "update_widget_a: error 19" combined_id single_id (Leaf_widget next_id (Editor window_id block_number new_row_number (min (max 0 (DS.length new_seq_seq_char-new_row_number)) row) font_size new_font_size True path find typesetting text_red text_green text_blue text_alpha cursor_red cursor_green cursor_blue cursor_alpha select_red select_green select_blue select_alpha height block_width delta_height x y extra_width extra_height ime_left ime_right ime_up ime_down new_font_height new_block_width new_delta_height (window_x+div (x*size) design_size-div (fromIntegral block_number*new_block_width) 2) (window_y+div (y*size) design_size-half_height) (window_x+div ((x-extra_width)*size) design_size-half_width) (window_x+div ((x+extra_width)*size) design_size+half_width) (window_y+div ((y-extra_height)*size) design_size-half_height) (window_y+div ((y+extra_height)*size) design_size+half_height) (window_x+div ((x+ime_left)*size) design_size) (window_x+div ((x+ime_right)*size) design_size) (window_y+div ((y+ime_up)*size) design_size) (window_y+div ((y+ime_down)*size) design_size) Cursor_none new_seq_seq_char editor_binding)) (error_replace_replace "update_widget_a: error 20" "update_widget_a: error 21" font_combined_id font_single_id (Leaf_widget font_next_id (Block_font font_window_id font_red font_green font_blue font_alpha (DIS.insert new_font_size (this_font,new_font_height,new_intmap_texture) font))) widget))
    _->error "update_widget_a: error 22"

update_editor::GS.HasCallStack=>Bool->SRT.Renderer->Int->Int->Int->FP.Ptr Color->DW.Word8->DW.Word8->DW.Word8->DW.Word8->FP.Ptr SRF.Font->FCT.CInt->DIS.IntMap (SRT.Texture,DIS.IntMap (Int,FCT.CInt),FCT.CInt,DW.Word8,DW.Word8,DW.Word8,DW.Word8)->DS.Seq (Char,Int,FCT.CInt)->DS.Seq (Char,Int,FCT.CInt)->DS.Seq (DS.Seq (Char,Int,FCT.CInt),Int,Int,Bool)->DS.Seq (DS.Seq (Char,Int,FCT.CInt),Int,Int,Bool)->IO (DIS.IntMap (SRT.Texture,DIS.IntMap (Int,FCT.CInt),FCT.CInt,DW.Word8,DW.Word8,DW.Word8,DW.Word8),DS.Seq (DS.Seq (Char,Int,FCT.CInt),Int,Int,Bool))
