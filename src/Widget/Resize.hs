...
update_widget_a seq_id start_id window widget=let (combined_id,single_id)=get_widget_id_widget seq_id start_id widget in case error_lookup_lookup "update_widget_a: error 1" "update_widget_a: error 2" combined_id single_id widget of
    Leaf_widget next_id (Rectangle window_id red green blue alpha left right up down _ _ _ _)->let (x,y,design_size,size)=get_adaptive_window window_id window in return (error_replace_replace "update_widget_a: error 3" "update_widget_a: error 4" combined_id single_id (Leaf_widget next_id (Rectangle window_id red green blue alpha left right up down (x+div (left*size) design_size) (y+div (up*size) design_size) (div ((right-left)*size) design_size) (div ((down-up)*size) design_size))) widget)
    ...
    Leaf_widget next_id (Editor window_id block_number _ row font_size _ _ path find typesetting text_red text_green text_blue text_alpha cursor_red cursor_green cursor_blue cursor_alpha select_red select_green select_blue select_alpha height block_width delta_height x y extra_width extra_height ime_left ime_right ime_up ime_down _ _ _ _ _ _ _ _ _ _ _ _ _ _ seq_seq_char editor_binding)->let (renderer,window_x,window_y,design_size,size)=get_renderer_with_adaptive_window window_id window in let new_block_width=div (block_width*size) design_size in let (font_next_id,font_combined_id,font_single_id,new_font_size,font_window_id,font_red,font_green,font_blue,font_alpha,font,this_font,_,intmap_texture)=find_block_font find widget block_width new_block_width design_size size path start_id font_size in FMA.alloca $ \text_color->do
        ...
        case seq_seq_char of
            DS.Empty->error "update_widget_a: error 11"
            ((seq_char,_,_,end) DS.:<| other_seq_seq_char)->do
                ...
                return (error_replace_replace "update_widget_a: error 12" "update_widget_a: error 13" combined_id single_id (Leaf_widget next_id (Editor window_id block_number new_row_number (min (max 0 (DS.length new_seq_seq_char-new_row_number)) row) font_size new_font_size True path find typesetting text_red text_green text_blue text_alpha cursor_red cursor_green cursor_blue cursor_alpha select_red select_green select_blue select_alpha height block_width delta_height x y extra_width extra_height ime_left ime_right ime_up ime_down new_font_height new_block_width new_delta_height (window_x+div (x*size) design_size-div (fromIntegral block_number*new_block_width) 2) (window_y+div (y*size) design_size-half_height) (window_x+div ((x-extra_width)*size) design_size-half_width) (window_x+div ((x+extra_width)*size) design_size+half_width) (window_y+div ((y-extra_height)*size) design_size-half_height) (window_y+div ((y+extra_height)*size) design_size+half_height) (window_x+div ((x+ime_left)*size) design_size) (window_x+div ((x+ime_right)*size) design_size) (window_y+div ((y+ime_up)*size) design_size) (window_y+div ((y+ime_down)*size) design_size) Cursor_none new_seq_seq_char editor_binding)) (error_replace_replace "update_widget_a: error 14" "update_widget_a: error 15" font_combined_id font_single_id (Leaf_widget font_next_id (Block_font font_window_id font_red font_green font_blue font_alpha (DIS.insert new_font_size (this_font,new_font_height,new_intmap_texture) font))) widget))
    _->error "update_widget_a: error 16"
