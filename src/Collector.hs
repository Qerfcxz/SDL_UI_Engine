...
collect seq_id index intmap_request (Engine widget window window_map request key count_id start_id main_id)=let (combined_id,single_id)=get_widget_id_widget seq_id start_id widget in Engine (error_update_update "collect: error 1" "collect: error 2" combined_id single_id (collect_a index intmap_request) widget) window window_map request key count_id start_id main_id
...
deliver seq_id judge (Engine widget window window_map request key count_id start_id main_id)=let (combined_id,single_id)=get_widget_id_widget seq_id start_id widget in let (seq_request,new_widget)=DIS.alterF (deliver_a single_id judge) combined_id widget in Engine new_widget window window_map (request DS.>< seq_request) key count_id start_id main_id
...
deliver_with_retain seq_id judge (Engine widget window window_map request key count_id start_id main_id)=case get_widget_widget seq_id start_id widget of
    Leaf_widget _ (Collector intmap_request)->Engine widget window window_map (DIS.foldlWithKey' (\seq_request index this_request->if judge index this_request then seq_request DS.|> this_request else seq_request) request intmap_request) key count_id start_id main_id
    _->error "deliver_with_retain error 1"

deliver_with_flush::DS.Seq Int->(Int->Request a->Bool)->Engine a->Engine a
deliver_with_flush seq_id judge (Engine widget window window_map request key count_id start_id main_id)=let (combined_id,single_id)=get_widget_id_widget seq_id start_id widget in let (intmap_request,new_widget)=DIS.alterF (deliver_with_flush_a single_id) combined_id widget in Engine new_widget window window_map (DIS.foldlWithKey' (\seq_request index this_request->if judge index this_request then seq_request DS.|> this_request else seq_request) request intmap_request) key count_id start_id main_id
