...
to_cursor::Int->Int->Int->Typesetting->FCT.CInt->FCT.CInt->FCT.CInt->FCT.CInt->FCT.CInt->FCT.CInt->FCT.CInt->DIS.IntMap (SRT.Texture,DIS.IntMap (Int,FCT.CInt),FCT.CInt)->DS.Seq (DS.Seq (Char,Int,FCT.CInt),Int,Int,Bool)->(Int,Int,Int,FCT.CInt)
to_cursor block_number row_number row typesetting cursor_x cursor_y font_height block_width delta_height x y intmap_texture seq_seq_char=let cursor_row=row+max 0 (min (row_number-1) (fromIntegral (div (cursor_y+div delta_height 2-y) (font_height+delta_height)))) in case DS.lookup cursor_row seq_seq_char of
    ...
    Just (seq_char,number,_,_)->let new_x=cursor_x-x in let cursor_block=max 0 (min block_number (fromIntegral (div new_x block_width))) in let (number_block,number_char)=to_cursor_a new_x block_width intmap_texture seq_char cursor_block (typesetting_left typesetting number block_number) 0 in (row+max 0 (min (row_number-1) (fromIntegral (div (cursor_y+div delta_height 2-y) (font_height+delta_height)))),number_block,number_char,new_x)

to_cursor_a::FCT.CInt->FCT.CInt->DIS.IntMap (SRT.Texture,DIS.IntMap (Int,FCT.CInt),FCT.CInt)->DS.Seq (Char,Int,FCT.CInt)->Int->Int->Int->(Int,Int)
...
get_cursor_row (Cursor_double bool cursor_row_start _ _ cursor_row_end _ _ _ _)=if bool then Just cursor_row_start else Just cursor_row_end

cursor_left::Int->Typesetting->FCT.CInt->DS.Seq (DS.Seq (Char,Int,FCT.CInt),Int,Int,Bool)->Cursor->Maybe (Int,Maybe Cursor)
cursor_left _ _ _ _ Cursor_none=Nothing
cursor_left block_number typesetting block_width seq_seq_char (Cursor_single cursor_row cursor_block cursor_char _)=if cursor_char==0
    then if cursor_row==0 then Just (0,Nothing) else let new_cursor_row=cursor_row-1 in case DS.lookup new_cursor_row seq_seq_char of
        ...
        Just (_,number,char_number,_)->let new_cursor_block=typesetting_right typesetting number block_number in Just (new_cursor_row,Just (Cursor_single new_cursor_row new_cursor_block char_number (fromIntegral new_cursor_block*block_width)))
        ...
        Just (seq_char,number,_,_)->let new_cursor_char=cursor_char-1 in case DS.lookup new_cursor_char seq_char of
            ...
            Just (_,block,_)->let new_cursor_block=cursor_block-block in Just (cursor_row,Just (Cursor_single cursor_row new_cursor_block new_cursor_char (fromIntegral (typesetting_left typesetting number block_number+new_cursor_block)*block_width)))
cursor_left _ _ _ _ (Cursor_double _ cursor_row_start cursor_block_start cursor_char_start _ _ _ cursor_x_start _)=Just (cursor_row_start,Just (Cursor_single cursor_row_start cursor_block_start cursor_char_start cursor_x_start))

cursor_right::Int->Int->Typesetting->FCT.CInt->DS.Seq (DS.Seq (Char,Int,FCT.CInt),Int,Int,Bool)->Cursor->Maybe (Int,Maybe Cursor)
cursor_right _ _ _ _ _ Cursor_none=Nothing
cursor_right max_row block_number typesetting block_width seq_seq_char (Cursor_single cursor_row cursor_block cursor_char _)=case DS.lookup cursor_row seq_seq_char of
    Nothing->error "cursor_right: error 1"
    Just (seq_char,number,char_number,_)->if cursor_char==char_number
        then if cursor_row==max_row then Just (max_row,Nothing) else let new_cursor_row=cursor_row+1 in let new_cursor_block=typesetting_left typesetting number block_number in Just (new_cursor_row,Just (Cursor_single new_cursor_row new_cursor_block 0 (fromIntegral new_cursor_block*block_width)))
        else case DS.lookup cursor_char seq_char of 
            Nothing->error "cursor_right: error 2"
            Just (_,block,_)->let new_cursor_block=cursor_block+block in Just (cursor_row,Just (Cursor_single cursor_row new_cursor_block (cursor_char+1) (fromIntegral (typesetting_left typesetting number block_number+new_cursor_block)*block_width)))
cursor_right _ _ _ _ _ (Cursor_double _ _ _ _ cursor_row_end cursor_block_end cursor_char_end _ cursor_x_end)=Just (cursor_row_end,Just (Cursor_single cursor_row_end cursor_block_end cursor_char_end cursor_x_end))
