...
move_geometry_combined_widget move_x move_y engine (Leaf_widget next_id (Geometry window_id red green blue alpha geometry))=let (window_x,window_y,design_size,size)=get_adaptive window_id engine in case geometry of
    ...
    Rounded_rectangle kind design_left design_right design_up design_down design_radius _ _ _ _ radius->let new_left=design_left+move_x in let new_right=design_right+move_x in let new_up=design_up+move_y in let new_down=design_down+move_y in Leaf_widget next_id (Geometry window_id red green blue alpha (Rounded_rectangle kind new_left new_right new_up new_down design_radius (fromIntegral (window_x+div (new_left*size) design_size)) (fromIntegral (window_x+div (new_right*size) design_size-1)) (fromIntegral (window_y+div (new_up*size) design_size)) (fromIntegral (window_y+div (new_down*size) design_size-1)) radius))
    Ellipse kind design_left design_right design_up design_down _ _ radius_x radius_y->let new_left=design_left+move_x in let new_right=design_right+move_x in let new_up=design_up+move_y in let new_down=design_down+move_y in Leaf_widget next_id (Geometry window_id red green blue alpha (Ellipse kind new_left new_right new_up new_down (fromIntegral (window_x+div ((new_left+new_right)*size) (2*design_size))) (fromIntegral (window_y+div ((new_up+new_down)*size) (2*design_size))) radius_x radius_y))
    Polygon kind seq_point _ number->let new_seq_point=fmap (\(x,y)->(x+move_x,y+move_y)) seq_point in Leaf_widget next_id (Geometry window_id red green blue alpha (Polygon kind new_seq_point (fmap (\(x,y)->(fromIntegral (window_x+div (x*size) design_size),fromIntegral (window_y+div (y*size) design_size))) new_seq_point) number))
