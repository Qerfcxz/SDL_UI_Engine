...
create_engine start_id=Engine (DIS.singleton start_id DIS.empty) DIS.empty DIS.empty DSeq.empty DSet.empty (start_id+1) start_id
...
loop_engine_time time_event_type event engine=do
    new_engine@(Engine _ _ new_window_map _ key _ new_start_id new_main_id)<-run_request engine
    (maybe_key,this_event)<-get_event event new_window_map key (Just time_event_type)
    case this_event of
        Quit->clean_engine new_engine
        _->case maybe_key of
            Nothing->loop_engine_time time_event_type event (run_event new_start_id new_main_id (DSeq.singleton new_main_id) this_event new_engine)
            Just new_key->loop_engine_time time_event_type event (run_event new_start_id new_main_id (DSeq.singleton new_main_id) this_event (set_key new_key new_engine))
...
loop_engine event engine=do
    ...
    case this_event of
        Quit->clean_engine new_engine
        _->case maybe_key of
            Nothing->loop_engine event (run_event new_start_id new_main_id (DSeq.singleton new_main_id) this_event new_engine)
            Just new_key->loop_engine event (run_event new_start_id new_main_id (DSeq.singleton new_main_id) this_event (set_key new_key new_engine))

run_request::Data a=>Engine a->IO (Engine a)
run_request (Engine widget window window_map request key count_id start_id main_id)=case request of
    DSeq.Empty->return (Engine widget window window_map DSeq.Empty key count_id start_id main_id)
    (this_request DSeq.:<| other_request)->do
        new_engine<-do_request this_request (Engine widget window window_map other_request key count_id start_id main_id)
        run_request new_engine

run_event::Data a=>Int->Int->DSeq.Seq Int->Event->Engine a->Engine a
run_event start_id main_id single_id_history event engine@(Engine widget _ _ _ _ _ _ _)=case DIS.lookup start_id widget of
...
run_event_a combined_id single_id single_id_history intmap_combined_widget event engine=case DIS.lookup single_id intmap_combined_widget of
    ...
    Just combined_widget->let new_engine@(Engine new_widget _ _ _ _ _ _ _)=run_event_b combined_widget event engine in case DIS.lookup combined_id new_widget of
...
run_event_b (Node_widget _ main_single_id combined_id) event engine=run_event combined_id main_single_id (DSeq.singleton main_single_id) event engine
...
clean_engine (Engine widget window _ _ _ _ _ _)=do
