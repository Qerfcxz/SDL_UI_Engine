...
create_request new_request (Engine widget window window_map request key count_id start_id main_id)=Engine widget window window_map (request DS.|> new_request) key count_id start_id main_id
...
do_request (Create_window window_id window_name left right up down) (Engine widget window window_map request key count_id start_id main_id)=DB.useAsCString (DTE.encodeUtf8 window_name) $ \name->let width=right-left in let height=down-up in do
    ...
    let new_sdl_window_id=fromIntegral sdl_window_id in return (Engine widget (error_insert "do_request: error 4" window_id (Window new_sdl_window_id new_window renderer width height 0 0 1 1) window) (error_insert "do_request: error 5" (fromIntegral sdl_window_id) window_id window_map) request key count_id start_id main_id)
do_request (Remove_window window_id) (Engine widget window window_map request key count_id start_id main_id)=case DIS.updateLookupWithKey (\_ _->Nothing) window_id window of
    ...
    (Just (Window sdl_window_id this_window renderer _ _ _ _ _ _),new_window)->do
        ...
        return (Engine widget new_window (simple_error_remove "do_request: error 7" sdl_window_id window_map) request key count_id start_id main_id)
...
do_request (Resize_window window_id left right up down) (Engine widget window window_map request key count_id start_id main_id)=let width=right-left in let height=down-up in case DIS.updateLookupWithKey (\_ (Window sdl_window_id sdl_window renderer design_width design_height _ _ _ _)->let (x,y,design_size,size)=adaptive_window design_width design_height width height in Just (Window sdl_window_id sdl_window renderer design_width design_height x y design_size size)) window_id window of
    ...
    (Just (Window _ sdl_window _ _ _ _ _ _ _),new_window)->do
        ...
        return (Engine widget new_window window_map request key count_id start_id main_id)
...
do_request (Render_text_widget seq_id) (Engine widget window window_map request key count_id start_id main_id)=do
    ...
    case combined_widget of
        Leaf_widget _ (Text window_id row _ _ _ _ _ _ _ _ _ _ left _ up down _ seq_row)->case DS.drop row seq_row of
            DS.Empty->return (Engine new_widget window window_map request key count_id start_id main_id)
            (new_row DS.:<| other_seq_row)->let renderer=get_renderer_window window_id window in case new_row of
                Row seq_texture y font_height->if down<up+font_height then return (Engine new_widget window window_map request key count_id start_id main_id) else let new_up=up-y in FMA.alloca $ \rect->do
                ...
                Row_blank y font_height->if down<up+font_height then return (Engine new_widget window window_map request key count_id start_id main_id) else let new_up=up-y in FMA.alloca $ \rect->do
                    render_seq_row rect left new_up down renderer other_seq_row
                    return (Engine new_widget window window_map request key count_id start_id main_id)
        _->error "do_request: error 24"
do_request (Render_editor_widget seq_id) (Engine widget window window_map request key count_id start_id main_id)=do
    ...
    case combined_widget of
        Leaf_widget _ (Editor window_id block_number row_number row _ font_size _ path _ typesetting text_red text_green text_blue text_alpha cursor_red cursor_green cursor_blue cursor_alpha select_red select_green select_blue select_alpha _ _ _ _ _ _ _ font_height block_width delta_height x y _ _ _ _ cursor seq_seq_char)->let (new_combined_id,new_single_id)=get_widget_id_widget path start_id new_widget in do
            ...
            return (Engine new_new_widget window window_map request key count_id start_id main_id)
        _->error "do_request: error 30"
do_request (Update_block_font_widget seq_id size block_width seq_char) (Engine widget window window_map request key count_id start_id main_id)=let (combined_id,single_id)=get_widget_id_widget seq_id start_id widget in do
