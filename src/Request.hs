...
do_request (Request rough_request instruction) engine=case rough_request of
    ...
    Remove_widget direct only seq_id->let (combined_id,single_id,transform)=get_widget_id_with_transform seq_id engine in if direct
        then case DF.foldlM (\this_instruction this_transform->this_transform engine rough_request this_instruction) instruction transform of
            ...
            Just new_instruction->let new_transform combined_widget=DF.foldlM (\mix this_instruction->remove_widget_instruction this_instruction engine mix) combined_widget new_instruction in if only then remove_widget_only new_transform combined_id single_id engine else remove_widget new_transform combined_id single_id engine
        else let new_transform combined_widget=DF.foldlM (\mix this_instruction->remove_widget_instruction this_instruction engine mix) combined_widget instruction in if only then remove_widget_only new_transform combined_id single_id engine else remove_widget new_transform combined_id single_id engine
    Replace_widget direct combined_widget_request seq_id->let (combined_id,single_id,transform)=get_widget_id_with_transform seq_id engine in if direct
        then case DF.foldlM (\this_instruction this_transform->this_transform engine rough_request this_instruction) instruction transform of
            Nothing->return engine
            Just new_instruction->let new_transform combined_widget=DF.foldlM (\mix this_instruction->replace_widget_instruction this_instruction engine mix) combined_widget new_instruction in replace_widget new_transform combined_id single_id combined_widget_request engine
        else let new_transform combined_widget=DF.foldlM (\mix this_instruction->replace_widget_instruction this_instruction engine mix) combined_widget instruction in replace_widget new_transform combined_id single_id combined_widget_request engine
    Alter_widget direct combined_widget_request seq_id->let (combined_id,single_id,transform)=get_widget_id_with_transform seq_id engine in if direct
        then case DF.foldlM (\this_instruction this_transform->this_transform engine rough_request this_instruction) instruction transform of
            Nothing->return engine
            Just new_instruction->let new_transform combined_widget=DF.foldlM (\mix this_instruction->alter_widget_instruction this_instruction engine mix) combined_widget new_instruction in alter_widget new_transform combined_id single_id combined_widget_request engine
        else let new_transform combined_widget=DF.foldlM (\mix this_instruction->alter_widget_instruction this_instruction engine mix) combined_widget instruction in alter_widget new_transform combined_id single_id combined_widget_request engine
    ...
    Clear_window window_id red green blue alpha->do
        ...
        catch_error "do_request: error 8" 0 (SRV.setRenderDrawColor renderer new_red new_green new_blue new_alpha)
    ...
    Render_rectangle window_id red green blue alpha left right up down->do
        ...
        let renderer=get_renderer new_window_id engine
        catch_error "do_request: error 11" 0 (SRV.setRenderDrawColor renderer new_red new_green new_blue new_alpha)
        FMA.alloca $ \rect->do
    ...
    Render_rectangle_widget direct seq_id->let (combined_id,single_id,transform)=get_widget_id_with_transform seq_id engine in let widget=get_engine_widget engine in if direct
        then case DF.foldlM (\this_instruction this_transform->this_transform engine rough_request this_instruction) instruction transform of
            ...
            Just new_instruction->do
                combined_widget<-DF.foldlM (\mix this_instruction->move_render_rectangle this_instruction engine mix) (error_lookup_lookup "do_request: error n" "do_request: error n" combined_id single_id widget) new_instruction
                do_request_render_rectangle_widget combined_widget engine
        else do
            combined_widget<-DF.foldlM (\mix this_instruction->move_render_rectangle this_instruction engine mix) (error_lookup_lookup "do_request: error n" "do_request: error n" combined_id single_id widget) instruction
            do_request_render_rectangle_widget combined_widget engine
    Render_picture_widget direct seq_id->let (combined_id,single_id,transform)=get_widget_id_with_transform seq_id engine in let widget=get_engine_widget engine in if direct
        then case DF.foldlM (\this_instruction this_transform->this_transform engine rough_request this_instruction) instruction transform of
            Nothing->return engine
            Just new_instruction->do
                combined_widget<-DF.foldlM (\mix this_instruction->move_render_picture this_instruction engine mix) (error_lookup_lookup "do_request: error n" "do_request: error n" combined_id single_id widget) new_instruction
                do_request_render_picture_widget combined_widget engine
        else do
            combined_widget<-DF.foldlM (\mix this_instruction->move_render_picture this_instruction engine mix) (error_lookup_lookup "do_request: error n" "do_request: error n" combined_id single_id widget) instruction
            do_request_render_picture_widget combined_widget engine
    Render_text_widget direct seq_id->let (combined_id,single_id,transform)=get_widget_id_with_transform seq_id engine in if direct
        then case DF.foldlM (\this_instruction this_transform->this_transform engine rough_request this_instruction) instruction transform of
            Nothing->return engine
            Just new_instruction->do
                let (combined_widget,new_widget)=error_get_update_update "" "" combined_id single_id set_render_false (get_engine_widget engine)
